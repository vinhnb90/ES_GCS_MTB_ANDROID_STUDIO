...
public class AdapterCameraBN extends ArrayAdapter<LinkedHashMap<String, String>>{
	...
	public AdapterCameraBN(Context context, 
...
//			lnWidth.getLayoutParams().width = Activity_Camera_BN.size + 65;
			...
			if(position == Activity_Camera_BN.selected_index){
	...
	private class GCS_Filter extends Filter {
		...
		protected FilterResults performFiltering(CharSequence constraint) {
			...
			if (constraint != null && constraint.toString().length() > 0) {
				...
				for (int i = 0, l = Activity_Camera_BN.arrCustomer.size(); i < l; i++) {
					LinkedHashMap<String, String> lst = Activity_Camera_BN.arrCustomer.get(i);

					if (AdapterCameraBN.this.filter_col != null) {
						String val = null;
						// nếu ko phải tìm các kh chưa ghi
						if (!AdapterCameraBN.this.filter_col.equals("CHUA_GHI") && !AdapterCameraBN.this.filter_col.equals("DA_GHI")) {
							val = lst.get(AdapterCameraBN.this.filter_col);
							if(val != null ){
								val = val.toLowerCase();
							}
						}
						// kiểm tra theo tên ko dấu và có dấu
						if (AdapterCameraBN.this.filter_col.equals("TEN_KHANG") || AdapterCameraBN.this.filter_col.equals("DIA_CHI")) {
						...
						} else if (AdapterCameraBN.this.filter_col.equals("CHUA_GHI")) {
				...
				result.values = filt;
				Activity_Camera_BN.arrCustomer_Filter = filt;
				if(constraint.toString().equals("")){
					Activity_Camera_BN.arrCustomer_Filter = Activity_Camera_BN.arrCustomer;
				}
			} else {

				synchronized (this) {
					result.values = Activity_Camera_BN.arrCustomer;
					result.count = Activity_Camera_BN.arrCustomer.size();
				}
		...
		protected void publishResults(CharSequence constraint, FilterResults results) {
			try{
				...
				AdapterCameraBN.this.filtered = (ArrayList<LinkedHashMap<String, String>>) results.values;
				AdapterCameraBN.this.notifyDataSetChanged();
				clear();
				if(AdapterCameraBN.this.filtered == null)
					AdapterCameraBN.this.filtered = Activity_Camera_BN.arrCustomer_Filter;
				for (int i = 0, l = AdapterCameraBN.this.filtered.size(); i < l; i++)
					AdapterCameraBN.this.add(AdapterCameraBN.this.filtered.get(i));
				notifyDataSetInvalidated();
			} catch(Exception ex) {
				ex.toString();
//				try{
...
//					clear();
//					int l = Activity_Camera_BN.arrCustomer_Filter.size();
//					for (int i = 0; i < l; i++)
//						AdapterPager.this.add(Activity_Camera_BN.arrCustomer_Filter.get(i));
