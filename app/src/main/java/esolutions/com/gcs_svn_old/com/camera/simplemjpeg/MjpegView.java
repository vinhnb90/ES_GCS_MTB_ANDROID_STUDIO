...
//				Log.e("error destRect", ex.toString());
				return null;
			}
		}
...
//				Log.e("error makeFpsOverlay", ex.toString());
				return null;
			}
		}
			...
			while (mRun) {
				...
				if (surfaceDone) {
					try {
						...
						if (ret == -1) {
							((Activity_Camera) saved_context).setImageError();
							return;
						}

						destRect = destRect(bmp.getWidth(), bmp.getHeight());
						
//						Bitmap bmImage = Bitmap.createScaledBitmap(bmp, bmp.getWidth()/2, bmp.getHeight()/2, false);
						c = mSurfaceHolder.lockCanvas();
						synchronized (mSurfaceHolder) {
							c.drawBitmap(bmp, null, destRect, p);
//							Log.e("errrrrrrrrrrrrrr", "" + Common.getUsedMemorySize(0) + "/" + Common.getUsedMemorySize(1));
							if(Common.getUsedMemorySize(0) < 900000){
								freeCameraMemory();
								Common.clearApplicationData(((Activity_Camera) saved_context));
								System.gc();
//								Log.e("errrrrrrrrrrrrrr", "" + Common.getUsedMemorySize(0) + "/" + Common.getUsedMemorySize(1));
							}
							if (showFps) {
...
//			Log.e("error startPlayback", ex.toString());
		}
	}
...
//			Log.e("error resumePlayback", ex.toString());
		}
	}
...
//			Log.e("error stopPlayback", ex.toString());
		}
	}
...
//			Log.e("error freeCameraMemory", ex.toString());
		}
	}
...
//			Log.e("error surfaceChanged", ex.toString());
		}
	}
