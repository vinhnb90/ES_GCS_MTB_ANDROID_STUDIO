...
public class AsyncTaskLoadDaGhiBN extends AsyncTask<String, LinkedHashMap<String, String>, ArrayList<LinkedHashMap<String, String>>>{
	...
	public AsyncTaskLoadDaGhiBN(Activity activityParent){
		try{
	...
	protected ArrayList<LinkedHashMap<String, String>> doInBackground(String... fileNames) {
		try{
			...
			for (String fileName : fns) {
				if(!fileName.equals("")){
					Cursor c_lotrinh = connection.getDataRouteByFileName(comm.convertFile(fileName, connection));
					Cursor c = connection.getAllDataGCSsoft(fileName);
					if(!c.moveToFirst() || !c_lotrinh.moveToFirst()){
						c = connection.getAllDataGCS(fileName);
					}
					if (c.moveToFirst()) {
						do {
							stt++;
