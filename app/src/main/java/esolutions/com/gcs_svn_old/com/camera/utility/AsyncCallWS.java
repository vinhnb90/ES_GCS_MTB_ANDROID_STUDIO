...
public class AsyncCallWS {
    ...
    public AsyncCallWS() {
        this.URL_SERVICE = "http://" + comm.cfgInfo.getIP_SERVICE() + "/WSGCS/Service1.asmx";
    }
    ...
    private String DownloadSingleFile(String _DirPath, String _FileName) {
        ...
        try {
            ...
            SoapObject request = new SoapObject(ConstantVariables.NAMESPACE, METHOD_NAME);
            ...
            HttpTransportSE androidHttpTransport = new HttpTransportSE(Common.cfgInfo.getIP_SERVICE());

            androidHttpTransport.call(ConstantVariables.NAMESPACE + METHOD_NAME, envelope);
            ...
            while (Offset < FileSize) {
                try {
                    ...
                    androidHttpTransport.call(ConstantVariables.NAMESPACE + METHOD_NAME,
                            envelope);
    ...
    private String UploadSingleFile(String _FileName) {
        try {
            ...
            request.addProperty("Offset", offset);
            request.addProperty("MA_DVIQLY", Common.MA_DVIQLY);
            request.addProperty("IMEI", Common.IMEI);
    ...
    private String UpdateServerFromFile(String[] _arrFileName) throws Exception {
        ...
        SoapObject request = new SoapObject(ConstantVariables.NAMESPACE, METHOD_NAME);
        request.addProperty("MA_DVIQLY", Common.MA_DVIQLY);
        request.addProperty("MA_NVGCS", Common.MA_NVGCS);
        request.addProperty("IMEI", Common.IMEI);
        ...
        try {
            ...
            if (tableRow != null) {
                ...
                if (LOAI_NV == 2 || LOAI_NV == 0 || LOAI_NV == 3) {
                    Common.MA_DVIQLY = tableRow.getProperty("MA_DVIQLY")
                            .toString();
                    Common.MA_NVGCS = tableRow.getProperty("MA_NV").toString();
                    Common.MA_DQL = tableRow.getProperty("MA_DQL").toString();
                } else {
    ...
    /**
     * Cập nhật 1 bản ghi chỉ số lên server
     *
     * @param MA_DVIQLY
     * @param MA_NVGCS
     * @param lst
     ...
     * @return
     */
    public String UploadRows(String MA_DVIQLY, String MA_NVGCS,
                             List<row_gcs> lst, String TEN_FILE) {

        try {
            if (lst.equals(null) && lst.size() < 1)
                return "Không có dữ liệu";

            // cập nhật dòng
            XStream xstream = new XStream();
            String METHOD_NAME = "UploadRows";
            SoapObject request = new SoapObject(ConstantVariables.NAMESPACE, METHOD_NAME);

            request.addProperty("MA_DVIQLY", MA_DVIQLY);
            request.addProperty("MA_NVGCS", MA_NVGCS);
            request.addProperty("IMEI", Common.IMEI);
            request.addProperty("xmlData", xstream.toXML(lst));
            request.addProperty("TEN_FILE", TEN_FILE);
...
//			String fileFullname = Environment.getExternalStorageDirectory().toString() + "/ESGCS/DB/ESGCS.s3db";
            ...
            SoapObject request = new SoapObject(ConstantVariables.NAMESPACE, METHOD_NAME);
            request.addProperty("FileName", "ESGCS_" + Common.IMEI + ".zip"); // đặt tên file khi tải lên server
            request.addProperty("buffer", bloc);
            request.addProperty("Offset", offset);
            request.addProperty("MA_DVIQLY", Common.MA_DVIQLY);
            request.addProperty("IMEI", Common.IMEI);
            ...
            return "Gửi dữ liệu thất bại: " + e.getMessage();
        }
    ...
    public String WS_UPLOAD_ROWS_CALL(String MA_DVIQLY, String MA_NVGCS,
                                      List<row_gcs> lst, String TEN_FILE) {
        WS_UPLOAD_ROWS ws_upload_rows = new WS_UPLOAD_ROWS(MA_DVIQLY, MA_NVGCS,
                lst, TEN_FILE);
        ws_upload_rows.execute();
        try {
            return ws_upload_rows.get();
        } catch (InterruptedException e) {
            return "FAIL";
        } catch (ExecutionException e) {
            return "FAIL";
    ...
    public String WS_UPLOAD_SO_GCS_CALL(String MA_DVIQLY, String MA_NVGCS,
        ...
        try {
            ...
            return "Lỗi kết nối với máy chủ. Nội dung lỗi: " + e.getMessage();
        } catch (ExecutionException e) {
            return "Lỗi kết nối với máy chủ. Nội dung lỗi: " + e.getMessage();
        }
    }
    ...
    public String WS_UPLOAD_SO_GCS_CALL_NEW(String MA_DVIQLY, String MA_NVGCS,
                                            String[] MaSo, View decorView, TextView tvCountUpload, TextView tvNotifyUpload, ProgressBar pbarUpload) {
        WS_UPLOAD_SO_GCS_NEW ws = new WS_UPLOAD_SO_GCS_NEW(MaSo, decorView, tvCountUpload, tvNotifyUpload, pbarUpload);
        ws.execute();
        try {
            return ws.get();
        } catch (InterruptedException e) {
    ...
    private class WS_UPLOAD_ROWS extends AsyncTask<String, Integer, String> {
        List<row_gcs> obj;
        String MA_NVGCS;
        String MA_DVIQLY;
        String TEN_FILE;

        public WS_UPLOAD_ROWS(String MA_DVIQLY, String MA_NVGCS,
                              List<row_gcs> obj, String TEN_FILE) {
            this.obj = obj;
        ...
        protected String doInBackground(String... params) {
            ...
            return UploadRows(MA_DVIQLY, MA_NVGCS, obj, TEN_FILE);
        }
    ...
    private class WS_UPLOAD_SO_GCS_NEW extends AsyncTask<String, Object, String> {
        ProgressBar pbarUpload;
        String[] ARRAY_TEN_FILE;
        View decorView;
        TextView tvCountUpload;
        TextView tvNotifyUpload;

        public WS_UPLOAD_SO_GCS_NEW(String[] ARRAY_TEN_FILE, View decorView, TextView tvCountUpload, TextView tvNotifyUpload, ProgressBar pbarUpload) {
            this.ARRAY_TEN_FILE = ARRAY_TEN_FILE;
            this.decorView = decorView;
            this.tvCountUpload = tvCountUpload;
            this.tvNotifyUpload = tvNotifyUpload;
            this.pbarUpload = pbarUpload;
        }
        ...
        protected String doInBackground(String... params) {
            ...
            try {
                if (ARRAY_TEN_FILE == null || ARRAY_TEN_FILE.length == 0)
                    return "Gửi sổ lên máy chủ thất bại có thể do: \n- Chưa chọn sổ \n- Sổ định đẩy lên server có thể đang bị khóa \n- Sổ không có trên máy";
...
//			String fileFullname = Environment.getExternalStorageDirectory().toString() + "/ESGCS/DB/ESGCS.s3db";
                ...
                HttpTransportSE androidHttpTransport = new HttpTransportSE(AsyncCallWS.URL_SERVICE);
                // androidHttpTransport.debug = true;
                ...
                while (offset < FileSize) {
                    ...
                    if (result != null)
                        return result;
//					else {
//						publishProgress("Lỗi kết nối khi đang gửi dữ liệu, kiểm tra lại mạng internet!");
...
//					}
                ...
                Thread.sleep(500);
                publishProgress(" Đang Cập nhật các sổ vừa đẩy lên vào dữ liệu máy chủ");
                // Update to server
                try {
                    Integer.parseInt(UpdateServerFromFile(ARRAY_TEN_FILE)); // cập nhật các sổ vừa đẩy lên vào server
                } catch (Exception ex) {
                    ex.printStackTrace();
                    publishProgress("Gặp lỗi khi cập nhật các sổ lên máy chủ tại API UpdateGCS_CHISO_HHU");
                    messageResponse = "Gặp lỗi khi cập nhật các sổ lên máy chủ tại API UpdateGCS_CHISO_HHU. \nNội dung lỗi: " + ex.getMessage();
                    throw new Exception(messageResponse);
                }

                messageResponse = "Cập nhật thành công " + ARRAY_TEN_FILE.length + " sổ";
...
//			return UploadSoGCS_NEW(View decorView, TextView tvCountUpload, TextView tvNotifyUpload, ARRAY_TEN_FILE);
        }
        ...
        protected void onProgressUpdate(final Object... values) {
            ...
            if (values[0] instanceof Integer) {
                decorView.post(new Runnable() {
                    @Override
                    public void run() {
                        tvCountUpload.setText((int) values[0] + "%");
                        tvNotifyUpload.setText("Đang gửi các sổ lên máy chủ...");
                        pbarUpload.setProgress((int) values[0]);
                    }
                });
            ...
            if (values[0] instanceof String) {
                decorView.post(new Runnable() {
                    @Override
                    public void run() {
                        tvCountUpload.setText(50 + "%");
                        tvNotifyUpload.setText(values[0] + "");
                        pbarUpload.setProgress(50);
                    }
                });
            }
        }
