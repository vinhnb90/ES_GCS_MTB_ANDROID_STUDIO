...
public class AsyncTaskCameraBN extends AsyncTask<String, LinkedHashMap<String, String>, ArrayList<LinkedHashMap<String, String>>>{

	SQLiteConnection connection;
	public Activity activityParent;
//	public ListView ((Activity_Camera_BN) activityParent).lvCustomer = null;
	public TextView tvDaGhi = null;
	public TextView tvMaQuyen = null;
//	AdapterCameraBN adapter = null;
	public int daghi;
	public int _stt = 0;
	...
	public AsyncTaskCameraBN(Activity activityParent){
		try{
			this.activityParent = activityParent;
			Activity_Camera_BN.arrCustomer = new ArrayList<LinkedHashMap<String,String>>();
			connection = new SQLiteConnection(activityParent, "ESGCS.s3db", Environment.getExternalStorageDirectory() + Common.DBFolderPath);
			((Activity_Camera_BN) activityParent).lvCustomer = (ListView)this.activityParent.findViewById(R.id.hlvCustomer);
			((Activity_Camera_BN) activityParent).adapter = new AdapterCameraBN(activityParent, Activity_Camera_BN.arrCustomer, R.layout.listview_gcs);
	...
	@Override
	protected ArrayList<LinkedHashMap<String, String>> doInBackground(String... fileNames) {
		try{
			daghi = 0;
			int stt = 0;
			Float dtt = 0F;
			...
			String fileName = "";
			String[] fns = fileNames[0].split(":");
			for (int i = 0; i < fns.length; i++) {
				if(i < fns.length - 1){
					fileName += "MA_QUYEN = '" + fns[i] + "' OR ";
				} else {
					fileName += "MA_QUYEN = '" + fns[i] + "'";
				}
			}
			
			if(!fileName.equals("")){
//				Cursor c_lotrinh = connection.getDataRouteByFileName(Common.convertFile(fileName, connection));
//				Cursor c = connection.getAllDataGCSsoft(fileName);
//				if(!c.moveToFirst() || !c_lotrinh.moveToFirst()){
				Cursor c = connection.getAllDataGCSBN(fileName);
//				}
				if (c.moveToFirst()) {
					do {
						stt++;
	...
	protected void onPostExecute(final ArrayList<LinkedHashMap<String, String>> result) {
		...
		try{
			...
			((Activity_Camera_BN) activityParent).setDataOnEditText(Activity_Camera_BN.selected_index, 3);
			...
			((Activity_Camera_BN) activityParent).lvCustomer.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
					...
					((Activity_Camera_BN) activityParent).setImage();
					((Activity_Camera_BN) activityParent).setDataOnEditText(Activity_Camera_BN.selected_index, 4);
					((Activity_Camera_BN) activityParent).showHidePmax(Activity_Camera_BN.selected_index);
					tvMaQuyen.setText(result.get(position).get("MA_QUYEN"));
				}
			});
			((Activity_Camera_BN) activityParent).lvCustomer.setOnItemLongClickListener(new OnItemLongClickListener() {
				...
				public boolean onItemLongClick(AdapterView<?> parent,
					...
					((Activity_Camera_BN) activityParent).showHidePmax(Activity_Camera_BN.selected_index);
					tvMaQuyen.setText(result.get(0).get("MA_QUYEN"));
					((Activity_Camera_BN) activityParent).lvCustomer.setSelection(0);
					return false;
				}
			});
		} catch(Exception ex) {
	...
	protected void onProgressUpdate(LinkedHashMap<String, String>... values) {
		...
		try{
			Activity_Camera_BN.arrCustomer.add(Activity_Camera_BN.arrCustomer.size() - Integer.parseInt(values[1].get("stt")), values[0]);
//			((Activity_Camera_BN) activityParent).adapter.notifyDataSetChanged();
//			((Activity_Camera_BN) activityParent).lvCustomer.invalidate();
			((Activity_Camera_BN) activityParent).adapter = new AdapterCameraBN(activityParent, Activity_Camera_BN.arrCustomer, R.layout.listview_gcs);
			((Activity_Camera_BN) activityParent).lvCustomer.setAdapter(((Activity_Camera_BN) activityParent).adapter);
