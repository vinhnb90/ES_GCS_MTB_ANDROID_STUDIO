...
public class AdapterCameraAiBall extends ArrayAdapter<LinkedHashMap<String, String>>{
	...
	public AdapterCameraAiBall(Context context, 
...
//			lnWidth.getLayoutParams().width = Activity_Camera_AiBall.size + 65;
			...
			if(position == Activity_Camera_AiBall.selected_index){
	...
	private class GCS_Filter extends Filter {
		...
		protected FilterResults performFiltering(CharSequence constraint) {
			...
			if (constraint != null && constraint.toString().length() > 0) {
				...
				for (int i = 0, l = Activity_Camera_AiBall.arrCustomer.size(); i < l; i++) {
					LinkedHashMap<String, String> lst = Activity_Camera_AiBall.arrCustomer.get(i);

					if (AdapterCameraAiBall.this.filter_col != null) {
						String val = null;
						// nếu ko phải tìm các kh chưa ghi
						if (!AdapterCameraAiBall.this.filter_col.equals("CHUA_GHI") && !AdapterCameraAiBall.this.filter_col.equals("DA_GHI")) {
							val = lst.get(AdapterCameraAiBall.this.filter_col);
							if(val != null ){
								val = val.toLowerCase();
							}
						}
						// kiểm tra theo tên ko dấu và có dấu
						if (AdapterCameraAiBall.this.filter_col.equals("TEN_KHANG") || AdapterCameraAiBall.this.filter_col.equals("DIA_CHI")) {
						...
						} else if (AdapterCameraAiBall.this.filter_col.equals("CHUA_GHI")) {
				...
				result.values = filt;
				Activity_Camera_AiBall.arrCustomer_Filter = filt;
				if(constraint.toString().equals("")){
					Activity_Camera_AiBall.arrCustomer_Filter = Activity_Camera_AiBall.arrCustomer;
				}
			} else {

				synchronized (this) {
					result.values = Activity_Camera_AiBall.arrCustomer;
					result.count = Activity_Camera_AiBall.arrCustomer.size();
				}
		...
		protected void publishResults(CharSequence constraint, FilterResults results) {
			try{
				...
				AdapterCameraAiBall.this.filtered = (ArrayList<LinkedHashMap<String, String>>) results.values;
				AdapterCameraAiBall.this.notifyDataSetChanged();
				clear();
				if(AdapterCameraAiBall.this.filtered == null)
					AdapterCameraAiBall.this.filtered = Activity_Camera_AiBall.arrCustomer_Filter;
				for (int i = 0, l = AdapterCameraAiBall.this.filtered.size(); i < l; i++)
					AdapterCameraAiBall.this.add(AdapterCameraAiBall.this.filtered.get(i));
				notifyDataSetInvalidated();
			} catch(Exception ex) {
				ex.toString();
//				try{
...
//					clear();
//					int l = Activity_Camera_AiBall.arrCustomer_Filter.size();
//					for (int i = 0; i < l; i++)
//						AdapterPager.this.add(Activity_Camera_AiBall.arrCustomer_Filter.get(i));
