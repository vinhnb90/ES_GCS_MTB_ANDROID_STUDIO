...
public class AdapterCameraMTB extends ArrayAdapter<LinkedHashMap<String, String>>{
	...
	public AdapterCameraMTB(Context context, 
			ArrayList<LinkedHashMap<String, String>> list_view, int resource) {

		super(context, R.layout.listview_camera_mtb, list_view);
	...
	public View getCustomView(int position, View convertView, ViewGroup parent) {
		...
		convertView = inflater.inflate(R.layout.listview_camera_mtb, null);
...
//			lnWidth.getLayoutParams().width = Activity_Camera_MTB.size + 65;
			...
			if(position == Activity_Camera_MTB.selected_index){
	...
	private class GCS_Filter extends Filter {
		...
		protected FilterResults performFiltering(CharSequence constraint) {
			...
			if (constraint != null && constraint.toString().length() > 0) {
				...
				for (int i = 0, l = Activity_Camera_MTB.arrCustomer.size(); i < l; i++) {
					LinkedHashMap<String, String> lst = Activity_Camera_MTB.arrCustomer.get(i);

					if (AdapterCameraMTB.this.filter_col != null) {
						String val = null;
						// nếu ko phải tìm các kh chưa ghi
						if (!AdapterCameraMTB.this.filter_col.equals("CHUA_GHI") && !AdapterCameraMTB.this.filter_col.equals("DA_GHI")) {
							val = lst.get(AdapterCameraMTB.this.filter_col);
							if(val != null ){
								val = val.toLowerCase();
							}
						}
						// kiểm tra theo tên ko dấu và có dấu
						if (AdapterCameraMTB.this.filter_col.equals("TEN_KHANG") || AdapterCameraMTB.this.filter_col.equals("DIA_CHI")) {
						...
						} else if (AdapterCameraMTB.this.filter_col.equals("CHUA_GHI")) {
				...
				result.values = filt;
				Activity_Camera_MTB.arrCustomer_Filter = filt;
				if(constraint.toString().equals("")){
					Activity_Camera_MTB.arrCustomer_Filter = Activity_Camera_MTB.arrCustomer;
				}
			} else {

				synchronized (this) {
					result.values = Activity_Camera_MTB.arrCustomer;
					result.count = Activity_Camera_MTB.arrCustomer.size();
				}
		...
		protected void publishResults(CharSequence constraint, FilterResults results) {
			try{
				...
				AdapterCameraMTB.this.filtered = (ArrayList<LinkedHashMap<String, String>>) results.values;
				AdapterCameraMTB.this.notifyDataSetChanged();
				clear();
				if(AdapterCameraMTB.this.filtered == null)
					AdapterCameraMTB.this.filtered = Activity_Camera_MTB.arrCustomer_Filter;
				for (int i = 0, l = AdapterCameraMTB.this.filtered.size(); i < l; i++)
					AdapterCameraMTB.this.add(AdapterCameraMTB.this.filtered.get(i));
				notifyDataSetInvalidated();
			} catch(Exception ex) {
				ex.toString();
//				try{
...
//					clear();
//					int l = Activity_Camera_MTB.arrCustomer_Filter.size();
//					for (int i = 0; i < l; i++)
//						AdapterPager.this.add(Activity_Camera_MTB.arrCustomer_Filter.get(i));
