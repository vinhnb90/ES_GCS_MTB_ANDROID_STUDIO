...
import static esolutions.com.gcs_svn_old.com.camera.ui.Activity_Camera.showDialogMessageOK;
...
//                    //check những KH ngoài lộ trình
//                    List<String> listMA_KHANG = connection.getKHNgoaiLoTrinh(FileName);
//                    if (listMA_KHANG.size() == 0)
//                        LoadListViewSortingRoute();
//                    else {
//                        showDialogMessageOK("Có thêm khách hàng mới ngoài danh sách lộ trình. \nCần làm mới lại lộ trình. Những khách hàng mới ngoài lộ trình tạm thời sắp xếp dưới cùng.", new IDialog() {
//                            @Override
...
//            Cursor c = connection.getAllDataGCSsoft(FileNameSelected);
            Cursor c = connection.getAllDataGCSsoftVinhTuy(FileNameSelected);

            if (!c.moveToFirst() || !c_lotrinh.moveToFirst()) {
//                c = connection.getAllDataGCS(FileNameSelected);
                c = connection.getAllDataGCSVinhTuy(FileNameSelected);
            }
            int i = 0;
...
//            Cursor c2 = connection.getAllDataGCS(FileNameSelected);
            Cursor c2 = connection.getAllDataGCSVinhTuy(FileNameSelected);
