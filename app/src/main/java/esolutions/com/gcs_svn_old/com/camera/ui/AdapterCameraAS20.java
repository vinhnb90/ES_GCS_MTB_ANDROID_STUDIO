...
public class AdapterCameraAS20 extends ArrayAdapter<LinkedHashMap<String, String>>{
	...
	public AdapterCameraAS20(Context context, 
...
//			lnWidth.getLayoutParams().width = Activity_Camera_AS20.size + 65;
...
//			tvSLcu.setText(val.get("SL_CU"));
			tvSLcu.setText(val.get("SLUONG_3") + ", " + val.get("SLUONG_2") + ", " + val.get("SLUONG_1"));
			...
			if(position == Activity_Camera_AS20.selected_index){
	...
	private class GCS_Filter extends Filter {
		...
		protected FilterResults performFiltering(CharSequence constraint) {
			...
			if (constraint != null && constraint.toString().length() > 0) {
				...
				for (int i = 0, l = Activity_Camera_AS20.arrCustomer.size(); i < l; i++) {
					LinkedHashMap<String, String> lst = Activity_Camera_AS20.arrCustomer.get(i);

					if (AdapterCameraAS20.this.filter_col != null) {
						String val = null;
						// nếu ko phải tìm các kh chưa ghi
						if (!AdapterCameraAS20.this.filter_col.equals("CHUA_GHI") && !AdapterCameraAS20.this.filter_col.equals("DA_GHI")) {
							val = lst.get(AdapterCameraAS20.this.filter_col);
							if(val != null ){
								val = val.toLowerCase();
							}
						}
						// kiểm tra theo tên ko dấu và có dấu
						if (AdapterCameraAS20.this.filter_col.equals("TEN_KHANG") || AdapterCameraAS20.this.filter_col.equals("DIA_CHI")) {
						...
						} else if (AdapterCameraAS20.this.filter_col.equals("CHUA_GHI")) {
				...
				result.values = filt;
				Activity_Camera_AS20.arrCustomer_Filter = filt;
				if(constraint.toString().equals("")){
					Activity_Camera_AS20.arrCustomer_Filter = Activity_Camera_AS20.arrCustomer;
				}
			} else {

				synchronized (this) {
					result.values = Activity_Camera_AS20.arrCustomer;
					result.count = Activity_Camera_AS20.arrCustomer.size();
				}
		...
		protected void publishResults(CharSequence constraint, FilterResults results) {
			try{
				...
				AdapterCameraAS20.this.filtered = (ArrayList<LinkedHashMap<String, String>>) results.values;
				AdapterCameraAS20.this.notifyDataSetChanged();
				clear();
				if(AdapterCameraAS20.this.filtered == null)
					AdapterCameraAS20.this.filtered = Activity_Camera_AS20.arrCustomer_Filter;
				for (int i = 0, l = AdapterCameraAS20.this.filtered.size(); i < l; i++)
					AdapterCameraAS20.this.add(AdapterCameraAS20.this.filtered.get(i));
				notifyDataSetInvalidated();
			} catch(Exception ex) {
				ex.toString();
//				try{
...
//					clear();
//					int l = Activity_Camera_AS20.arrCustomer_Filter.size();
//					for (int i = 0; i < l; i++)
//						AdapterPager.this.add(Activity_Camera_AS20.arrCustomer_Filter.get(i));
