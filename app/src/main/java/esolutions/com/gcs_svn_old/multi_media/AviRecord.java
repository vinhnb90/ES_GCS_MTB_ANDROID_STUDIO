...
import esolutions.com.gcs_svn_old.utils.Util;
...
//    private String gsLocalPath;
	...
	public AviRecord(String filename, int resolutionJpg,boolean audioFlag)
	{
		m_filename = filename;
		m_curResolutonJpg = resolutionJpg; 
		setSizeOfJpgInResolution();
		m_ImageBuf = new byte[Define.MAX_IMAGE_BUF_NUMBER][Define.MAX_IMAGE_BUF_LEN];
		m_ImageBufLen = new int[Define.MAX_IMAGE_BUF_NUMBER];
		m_ImageBufStatus = new int[Define.MAX_IMAGE_BUF_NUMBER];
		
		m_AudioBuf = new byte[Define.MAX_AUDIO_BUR_NUMBER][Define.MAX_AUDIO_BUF_LEN];
		m_AudioBufLen = new int[Define.MAX_AUDIO_BUR_NUMBER];
		m_AudioBufStatus = new int[Define.MAX_AUDIO_BUR_NUMBER];
		resetBufferVideoAudio();
		
		m_recordFlag = Define.STOP;
		m_ImgCounter = 0;
		m_AudioCounter = 0;
	...
	private void setSizeOfJpgInResolution()
	{
		...
		case Define.RESOLUTON_VGA:
		{
			m_pwidth = 640;
			m_pheight= 480;
			break;
		}
		case Define.RESOLUTON_QVGA:
		{
			m_pwidth = 320;
			m_pheight= 240;
			break;
		}
		case Define.RESOLUTON_QQVGA:
		{
			m_pwidth = 160;
			m_pheight= 120;
			break;
	...
	{
		for(int i = 0 ; i < Define.MAX_IMAGE_BUF_NUMBER;i++)
		{
			m_ImageBufStatus[i]= Define.BUFFER_EMPTY;
			m_ImageBufLen[i] = 0;
		}
		m_Image_idx = 0;
		m_ImageRec_idx = 0;
		
		for(int i = 0 ; i < Define.MAX_AUDIO_BUR_NUMBER;i++)
		{
			m_AudioBufStatus[i]= Define.BUFFER_EMPTY;
			m_AudioBufLen[i] = 0;
		}
		m_Audio_idx = 0;
		m_AudioRec_idx = 0;
	...
	private void receiveAudio(byte[] audioData, int audioLen)
	{
		...
		{
			 int len;
			 if(i + Define.MAX_AUDIO_BUF_LEN <= audioLen)
				 len = Define.MAX_AUDIO_BUF_LEN;
			 else{
				 len = audioLen - i;
			 }
			 if(m_AudioBufStatus[m_AudioRec_idx] == Define.BUFFER_EMPTY)
			 {
				 m_AudioBufStatus[m_AudioRec_idx] = Define.BUFFER_PROCESSING;
				 System.arraycopy(audioData, i,m_AudioBuf[m_AudioRec_idx],0 , len);
				 m_AudioBufLen[m_AudioRec_idx] = len;
				 i += len;
				 
				 m_AudioBufStatus[m_AudioRec_idx] = Define.BUFFER_FULL;
				 m_AudioRec_idx = (m_AudioRec_idx + 1)% Define.MAX_AUDIO_BUR_NUMBER;
	...
	public void receiveImage(byte[] imgData, int imgLen)
	{
		while(true)
		{
			if(m_ImageBufStatus[m_ImageRec_idx] == Define.BUFFER_EMPTY)
			{
				m_ImageBufStatus[m_ImageRec_idx] = Define.BUFFER_PROCESSING;
				System.arraycopy(imgData, 0, m_ImageBuf[m_ImageRec_idx], 0, imgLen);
				m_ImageBufLen[m_ImageRec_idx] = imgLen;
				
				m_ImageBufStatus[m_ImageRec_idx] = Define.BUFFER_FULL;
				m_ImageRec_idx = (m_ImageRec_idx + 1) % Define.MAX_IMAGE_BUF_NUMBER;
				return;
			}else{
				try{
	...
	public void AviOpen(AVI_INFO avi_info)
    {
        ...
        {
            System.out.println("avi open fail 763 " + e);
        }
        try
        {
        ...
        {
            System.out.println("avi open fail 794 " + e);
        }
        try
        {
        ...
        {
            System.out.println("avi open fail 816 " + e);
        }
        try
        {
        ...
        {
            System.out.println("avi open fail 862 " + e);
        }
        try
        {
        ...
        {
            System.out.println("avi open fail 942" + e);
        }
    }
...
//		byte abyte0[] = new byte[2];
	...
	public void run()
	{
		...
		while(true)
		{
	        if(m_recordFlag == Define.RUN) 
	        {
	        	//System.out.println("Record : TotalFrameNumber-->"+Integer.toString(avi_info.mp4.m_mp4para.TotalFrameNumber) + "NumberCurrentVideo-->"+Integer.toString(avi_info.mp4.m_NumCurrentVideo));
//	            long l1 = 0L;
	            //m_FinishRecord = false;
	            ...
	            try
	            {
	                avi_info.fd = new RandomAccessFile(m_filename, "rw");      
	            }
	            catch(Exception e)
	            {
	            ...
	            do
	            {
	            	while(m_AudioBufStatus[m_Audio_idx] == Define.BUFFER_FULL)
	                {
                        ...
                        m_AudioBufStatus[m_AudioRec_idx] = Define.BUFFER_EMPTY;
	                ...
	                if(l == 0)
	                {
	                    ...
	                    //l1 = avi_info.first_video_frame_time_stamp / 1000L / 60L;
	                    l = 1;
	                }
	                if(avi_info.mp4.m_idxSizeWrited + 16 > avi_info.mp4.m_idxBuf.length)
            		{
	                ...
	                if(m_ImageBufStatus[m_Image_idx] == Define.BUFFER_FULL)
	                {
	                    ...
	                    m_ImageBufStatus[m_Image_idx] = Define.BUFFER_EMPTY;
	                ...
	                try {
						m_fileSize = avi_info.fd.getFilePointer();
					} catch (IOException e) {
						e.printStackTrace();
					}
	            } while(m_recordFlag == Define.RUN);
	            //m_FinishRecord = true;
